...
WHERE spec_definition_id IN (
  SELECT id FROM spec_definitions WHERE kind_id = 'kind-gpu'
);
...
INSERT INTO spec_definitions (id, kind_id, key, label, unit, value_type, higher_is_better, comparison_mode, comparison_role, comparison_weight, minimum_difference_percent, group_label, sort_order) VALUES
  ('gpu-spec-vram',       'kind-gpu', 'vram_gb',        'VRAM',           'GB',  'number',  1,    'numeric', 'primary',       3, 5, 'Memory',  10),
  ('gpu-spec-memtype',    'kind-gpu', 'memory_type',    'Memory Type',    NULL,  'text',    NULL, 'ordinal', 'informational', 0, 0, 'Memory',  20),
  ('gpu-spec-membus',     'kind-gpu', 'memory_bus_bit', 'Memory Bus',     'bit', 'number',  1,    'numeric', 'primary',       2, 5, 'Memory',  30),
  ('gpu-spec-boost',      'kind-gpu', 'boost_clock_mhz','Boost Clock',    'MHz', 'number',  1,    'numeric', 'informational', 0, 0, 'Clocks',  40),
  ('gpu-spec-cores',      'kind-gpu', 'shader_cores',   'Shader Cores',   NULL,  'number',  1,    'numeric', 'primary',       2, 5, 'Compute', 50),
  ('gpu-spec-tdp',        'kind-gpu', 'tdp_w',          'TDP',            'W',   'number',  0,    'numeric', 'tradeoff',      0, 0, 'Power',   60),
  ('gpu-spec-length',     'kind-gpu', 'length_mm',      'Length',         'mm',  'number',  0,    'numeric', 'tradeoff',      0, 0, 'Physical',70),
  ('gpu-spec-rt',         'kind-gpu', 'ray_tracing',    'Ray Tracing',    NULL,  'boolean', NULL, 'none',    'informational', 0, 0, 'Features',80),
  ('gpu-spec-msrp',       'kind-gpu', 'msrp_usd',       'MSRP',           '$',   'number',  0,    'numeric', 'tradeoff',      0, 0, 'Price',   90);

INSERT INTO spec_options (id, spec_definition_id, key, label, rank, sort_order) VALUES
  ('gpu-memtype-gddr6',  'gpu-spec-memtype', 'gddr6',  'GDDR6',  1, 10),
  ('gpu-memtype-gddr6x', 'gpu-spec-memtype', 'gddr6x', 'GDDR6X', 2, 20),
  ('gpu-memtype-gddr7',  'gpu-spec-memtype', 'gddr7',  'GDDR7',  3, 30);
